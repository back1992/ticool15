...
var MooCrop = new Class({
	...
	initialize: function(el, options){
		...
		if (!this.options.width || this.options.width < this.options.min.width) this.options.width = this.options.min.width; 
		if (!this.options.height || this.options.height < this.options.min.height) this.options.height = this.options.min.height;
	...
	setup: function(){
		$(this.cropArea).setStyles({
			...
			'top' : this.options.top != null ? this.options.top : (this.img.height - this.options.height)/2,
			'left': this.options.left != null ? this.options.left : (this.img.width - this.options.width) / 2 
		});
	...
	fixBoxModel : function(){
		var diff = this.boxDiff = (this.crop.width - this.options.width)/2;
	...
	buildOverlay: function(){
		var o = this.options;
		var imgCssFloat = this.img.getStyle('float');
		if (!imgCssFloat) imgCssFloat = '';
		this.wrapper = new Element("div", {
			'styles' : {'position' : 'relative', 'width' : this.img.width, 'height' : this.img.height, 'background' : 'url('+this.img.src+') no-repeat' , 'float' : imgCssFloat  }
