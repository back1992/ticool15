...
if(!empty($page_no)){
   ...
   for($y = ($page_no-$numberQns)+1;$y < $page_no+1; $y++){
      ...
      $_SESSION['field_name'.$y] = $_POST['field_name'.$y];
      $_SESSION['page_no'] = $y;
   }
...
{
   $check_msg = "";
   $qid = $_SESSION['question'.$i];

   $question = bfquiztrialController::getQuestion($qid);
   $answer = $_SESSION['q'.$qid];

   if($_SESSION['questiontype'.$i] == 1){ // radio
      if($answer == "_OTHER_"){
         $answer = $_SESSION['q'.$qid.'_OTHER_'];
      }
   }

   if($_SESSION['questiontype'.$i] == 2){ // checkbox
     if($answer==""){
         // do nothing
     }else{
   ...
   if($answer == ""){
      ...
      //uncomment this line if you want email to show all results, even if correct. (or comment out to hide)
      $body .= "\n".$question.": \n".JText::_("Answer").": ".$answer."\n";
   }

   if($_SESSION['field_name'.$i] == ""){
      // do nothing
   }else if($_SESSION['field_name'.$i] == "NULL"){
      // do nothing
   }else{ // save data to db
      bfquiztrialController::saveField($result,$_SESSION['field_name'.$i],$answer,$table);
      $score=$score+bfquiztrialController::getScore($_SESSION['field_name'.$i],$qntable,$answer);
      if($scoringMethod == 1){
         $answerSeq.=bfquiztrialController::getAnswerSeq($_SESSION['field_name'.$i],$answer);
      }
   }
}
...
<form action="index.php?option=com_bfquiztrial&view=bfquiztrial&Itemid=<?php echo $Itemid; ?>" method="POST" name="mysurvey" id="mysurvey" class="form-validate" class="form-validate" onSubmit="return myValidate(this);">
