<?php
/**
 * sayg Model for bfquiztrial Component
 *
 * @package    Joomla
 ...
 * @subpackage Components
 */
class bfquiztrialModelsayg extends JModel
{
	/**
	...
	function _buildQueryParent()
	{
		...
		$query = 'SELECT b.*,  cc.title AS category_name'
						. ' FROM #__bfquiztrial AS b'
						. ' LEFT JOIN #__categories AS cc ON cc.id = b.catid'
						. ' WHERE b.published AND parent=0 AND b.catid = '.$catid
						. ' ORDER BY b.parent, b.ordering'
		;
	...
	{
		//only get parent items
		// if data hasn't already been obtained, load it
		if (empty($this->_data)) {
		    $query = $this->_buildQueryParent();
		    $this->_data = $this->_getList( $query );
		}
