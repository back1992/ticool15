<?php
/**
 * bfquiztrial scorerange controller
 *
 * @package    Joomla
 ...
 * @subpackage Components
 */
class bfquiztrialControllerscorerange extends JController
{
	/**
	...
	*/
	function publishQuestion(  ) {
		bfquiztrialControllerscorerange::changePublishQuestion( 1 );
	}
	...
	*/
	function unPublishQuestion(  ) {
		bfquiztrialControllerscorerange::changePublishQuestion( 0 );
	}
	...
	*/
	function changePublishQuestion( $publish )
	{
		...
		$query = 'UPDATE #__bfquiztrial_scorerange'
...
function moveUpscorerange(  ) {
	bfquiztrialControllerscorerange::orderscorerange( -1 );
}
...
function orderscorerange( $inc )
{
	...
	$row =& JTable::getInstance('scorerangeanswer', 'Table');
	...
	function remove()
	{
		...
		if(!$model->delete()) {
			$msg = JText::_( 'Error: One or More ABCD Answer scorerange Could not be Deleted' );
		} else {
			$msg = JText::_( 'ABCD Answer scorerange(s) Deleted' );
		}

		$this->setRedirect( 'index.php?option=com_bfquiztrial&task=scorerange', $msg );
	}
	...
	function copy()
	{
		...
		$this->setRedirect( 'index.php?option=com_bfquiztrial&task=scorerange' );

		$cid	= JRequest::getVar( 'cid', null, 'post', 'array' );
		$db		=& JFactory::getDBO();

		$table	=& JTable::getInstance('scorerangeanswer', 'Table');
	...
	/**
	* Sets default ABCD  answer scorerange
	*/
	function setdefault()
	{
		global $mainframe;
		...
		//get category id
		$query = 'SELECT `catid` FROM #__bfquiztrial_scorerange'
		. ' WHERE `id` = ( '. $cid[0] .' )'
		;
		$db->setQuery( $query );
		$catid=$db->loadResult();
	...
	function saveOrder()
	{
		...
		$this->setRedirect( 'index.php?option=com_bfquiztrial&controller=scorerange&task=scorerange' );
