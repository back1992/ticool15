<?php
/**
 * scorerange Answer Model for bfquiztrial Component
 *
 * @package    Joomla
...
/**
 * scorerange Answer Model
 *
 * @package    Joomla
 * @subpackage Components
 */
class bfquiztrialModelscorerangeAnswer extends JModel
{
	/**
	...
	/**
	 * Method to set the scorerange Answer identifier
	 *
	 * @access	public
	...
	/**
	 * Method to get a scorerange Answer
	...
	function &getData()
	{
		// Load the data
		if (empty( $this->_data )) {
			$query = ' SELECT * FROM #__bfquiztrial_scorerange '.
					'  WHERE id = '.$this->_id;
	...
	function store()	{
		...
		if ($row->default == "") {
		   ...
		   $query = ' SELECT * FROM #__bfquiztrial_scorerange WHERE `catid`='.$row->catid.' AND `default`=1';
		   $this->_db->setQuery( $query );
		...
		if ($row->ordering == "0" | $row->ordering == "") {
		   // get next ordering
		   $query = ' SELECT MAX(ordering) as ordering FROM #__bfquiztrial_scorerange';
		   $this->_db->setQuery( $query );
		   $this->_mydata = $this->_db->loadObject();
		   $row->ordering = intval($this->_mydata->ordering)+1;
		}

		// Make sure the bfquiztrial scorerange Answer record is valid
		if (!$row->check()) {
			$this->setError($this->_db->getErrorMsg());
			return false;
		}
