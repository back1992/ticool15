    ...
    */
    ...
    class JElementGroups extends JElement {
        ...
        function fetchElement($name, $value, &$node, $control_name) {
            ...
            $document->addScriptDeclaration(sprintf('window.addEvent("domready", function(){ new ARIElementGroups("%s", %s); });', $containerId, AriJSONHelper::encode(array('selectId'=>$selectId))));
            return sprintf('<div id="%1$s"><fieldset><legend><label for="%2$s">%3$s</label>&nbsp;&nbsp;%4$s</legend><div>%5$s</div></fieldset></div>', $containerId, $selectId, JText::_($node->attributes('label')), JHTML::_('select.genericlist', $this->_getOptionsGroup($node), $control_name.'['.$name.']', 'inputbox', 'value', 'text', $value, $selectId), $childParameter->render());
        }
        ...
        function _includeAssets() {
            ...
            if (strlen(JPATH_ROOT) >1) $filePath = str_replace(JPATH_ROOT, '', $filePath);
        ...
        function _addGroupAttrs(&$node, $selectedGroup, $selectId) {
            if (empty($node->group)) return $options;
        ...
        function _getOptionsGroup(&$node) {
            ...
            foreach($node->group as $group) {
                $options[] = JHTML::_('select.option', $group->attributes('group_id'), JText::_($group->attributes('label')));
            }
            return $options;
        }
    }
    class JParameterGroups extends JParameter {
        function render($name = 'params', $group = '_default') {
            if (!isset($this->_xml[$group])) return false;
